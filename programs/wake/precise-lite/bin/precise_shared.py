...
def load_precise(args: argparse.Namespace) -> TFLiteHotWordEngine:
    return TFLiteHotWordEngine(
        local_model_file=Path(args.model).absolute(),
        sensitivity=args.sensitivity,
        trigger_level=args.trigger_level,
        chunk_size=args.samples_per_chunk * 2,  # 16-bit samples
    )
